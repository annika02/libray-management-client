...
const normalizeCategory = (category) => {
  ...
  return categoryMap[normalized] || "history";
};
  ...
  const fetchBooks = async () => {
    try {
      const response = await fetch(
        "https://library-server-alpha.vercel.app/history",
        {
          method: "GET",
          headers: {
      ...
      if (data && data.length > 0) {
        ...
        setError("No history books found.");
      }
    } catch (err) {
      console.error("Error fetching books:", err);
      setError("Failed to load history books.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (error) {
    return (
      <div className="container mx-auto p-4">
        <h2 className="text-2xl font-semibold mb-6">History Books</h2>
